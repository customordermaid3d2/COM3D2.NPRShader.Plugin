...
{
	internal class MaterialPane : BasePane
	{
		public MaterialPane(int fontSize, string catname, string slotname, int slotnum, string matefilename, Maid maid) : base(fontSize, string.Concat(new string[]
		{
			catname,
			":",
			slotname,
			...
			if (this.bLoadMateFileRequested)
			{
				...
				using (Dictionary<string, float>.KeyCollection.Enumerator enumerator3 = this.fProps.Keys.GetEnumerator())
				{
					while (enumerator3.MoveNext())
					{
						...
						float min = 0f;
						float max = 1f;
						MaterialPane.ButtonType buttonType = MaterialPane.ButtonType.Slider;
						...
						if (!s.ToLower().Contains("ztest2alpha"))
						{
							...
							{
								buttonType = MaterialPane.ButtonType.Toggle;
							}
							...
							{
								buttonType = MaterialPane.ButtonType.List;
							}
						}
						switch (buttonType)
						{
						case MaterialPane.ButtonType.Slider:
						{
						...
						case MaterialPane.ButtonType.Toggle:
						{
							bool value = num > 0f;
		...
		public void parseMateFile(string materialName, out Dictionary<string, Color> colProps, out Dictionary<string, float> fProps, out Dictionary<string, bool> sKeywords)
		{
			...
			{
				foreach (string text in MaterialPane.BASECOLORPROP)
				{
				...
				foreach (string text2 in MaterialPane.BASEFRPROP)
				{
				...
				foreach (string text3 in MaterialPane.SHADERKEYWORD)
				{
			...
			foreach (string text9 in MaterialPane.BASECOLORPROP)
			{
			...
			foreach (string text10 in MaterialPane.BASEFRPROP)
			{
			...
			foreach (string text11 in MaterialPane.SHADERKEYWORD)
			{
		...
		{
			int i = (int)TBody.hashSlotName[this.SlotName];
			GameObject obj = this.Maid.body0.goSlot[i].obj;
			foreach (SkinnedMeshRenderer skinnedMeshRenderer in obj.transform.GetComponentsInChildren<SkinnedMeshRenderer>(true))
			{
				bool flag = skinnedMeshRenderer != null && skinnedMeshRenderer.material != null && this.SlotNum < skinnedMeshRenderer.materials.Length;
				if (flag)
				{
		...
		{
			int index = (int)TBody.hashSlotName[this.SlotName];
			GameObject obj = this.Maid.body0.goSlot[index].obj;
			foreach (SkinnedMeshRenderer skinnedMeshRenderer in obj.transform.GetComponentsInChildren<SkinnedMeshRenderer>(true))
			{
				if (skinnedMeshRenderer != null && skinnedMeshRenderer.material != null && this.SlotNum < skinnedMeshRenderer.materials.Length)
				{
					...
					if (this.Material.shader.name.Contains("_Reflection_"))
					{
						skinnedMeshRenderer.reflectionProbeUsage = ReflectionProbeUsage.BlendProbesAndSkybox;
					}
					return;
				}
			}
		...
		public MaterialPane.ButtonType buttonType
		{
			get
			{
		...
		private CustomButton recalculateNormalsButton;

		public MaterialPane.ButtonType _buttonType;
		...
		private sealed class <>c
		{
			...
			public static readonly MaterialPane.<>c <>9 = new MaterialPane.<>c();
		...
		private sealed class <>c__DisplayClass2_2
		{
			...
			public MaterialPane.<>c__DisplayClass2_1 CS$<>8__locals1;
		}
		...
		private sealed class <>c__DisplayClass2_3
		{
			...
			public MaterialPane.<>c__DisplayClass2_1 CS$<>8__locals2;
		}
		...
		private sealed class <>c__DisplayClass2_4
		{
			...
			public MaterialPane.<>c__DisplayClass2_1 CS$<>8__locals3;
		}
